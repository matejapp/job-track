...
import { ChevronLeft, ExternalLink, Plus, Trash2, MoreVertical, Check, ClipboardList, Activity as ActivityIcon, StickyNote, Info } from 'lucide-react'
import { toast } from 'react-toastify'
import { getJobApplication, deleteApplication } from '@/api/applications'
...
import type { ActivityImportance } from '@/types'
...
function StageTracker({ status }: { status: string }) {
  const steps = ['Applied', 'Interview', 'Offer', 'Hired']
      ...
      <Tabs defaultValue="details">
        ...
        <TabsContent value="details" className="mt-4">
          <div className="grid grid-cols-2 gap-x-8 gap-y-5 p-4 rounded-lg border border-border bg-bg-surface">
            ...
            <div><p className="text-xs text-text-muted uppercase tracking-wide mb-1">Status</p><StatusBadge status={app.status} /></div>
            {app.applicationLink && (
